...
Set `FETCHHARBOR_ADMIN_ENABLED=true`, supply a strong token (preferably with `FETCHHARBOR_ADMIN_TOKEN_FILE`), and open the separately protected admin hostname; its root redirects to `/admin`. The consolidated dashboard provides process/host and per-route monitoring, editable allow-listed runtime settings, service pricing, security posture checks, and a payload-free configuration audit trail. Pricing and payout-address changes are persisted for the next restart so discovery metadata and payment enforcement cannot disagree during a running process.
